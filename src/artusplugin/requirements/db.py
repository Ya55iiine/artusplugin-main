...
schema = [
    # Requirements
    Table('requirement', key=('requirement', 'ticket'))[
        Column('requirement'),
...
def create_tables(cursor, env):
    ...
    for table in schema:
        for stmt in to_sql(env, table):
            cursor.execute(stmt)
    cursor.execute("INSERT into system values ('requirements_version', '1')")
...
class RequirementsSetup(Component):
    ...
    def _get_version(self, cursor):
        try:
            sql = "SELECT value FROM system WHERE name='requirements_version'"
            self.log.debug(sql)
            cursor.execute(sql)
            for row in cursor:
                return int(row[0])
